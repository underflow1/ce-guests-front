 ...
 * @param {number|null|undefined} state - состояние записи (10/20/30/40/50/60)
 * @returns {string|null} - класс иконки FontAwesome или null
 */
export function getMeetingResultIcon(state) {
  const s = Number(state)
  if (s === 10) return 'fa-user-check'
  if (s === 20) return 'fa-xmark'
  if (s === 30) return 'fa-user-check'
 ...
 * @returns {string|null} - вариант (off/accepted/pending/employed/cancelled) или null
 */
export function getMeetingResultVariant(state) {
  const s = Number(state)
  if (s === 10) return 'off'
  if (s === 20) return 'cancelled'
  if (s === 30) return 'accepted'
